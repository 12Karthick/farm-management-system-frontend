...
export default function FarmAdminsTab() {
  ...
  const farmAdminsList = useMemo(() => {
    if (!farmPersonsData) return [];
    return farmPersonsData
      .filter((fp) => fp.farmRole === 'Farm Admin')
      .map((fp) => {
  ...
  // Sync loaded database farm admins with local draft state
  useEffect(() => {
    if (farmPersonsData) {
      if (JSON.stringify(draft.farmAdmins) !== JSON.stringify(farmAdminsList)) {
        updateDraft('farmAdmins', farmAdminsList);
      }
    }
  }, [farmPersonsData, farmAdminsList, draft.farmAdmins, updateDraft]);
  ...
  async function handleLiveCreate(rec: RoleRecord) {
    try {
      ...
      message.success(`Farm Admin "${rec.name}" successfully created.`);
    } catch (err: any) {
      message.error(err?.data?.message || 'Failed to create Farm Admin.');
      throw err;
    }
  }
  ...
  async function handleLiveEdit(rec: RoleRecord) {
    try {
      ...
      message.success(`Farm Admin "${rec.name}" successfully updated.`);
    } catch (err: any) {
      message.error(err?.data?.message || 'Failed to update Farm Admin.');
      throw err;
    }
  }
  ...
  async function handleLiveDelete(id: string) {
    try {
      ...
      message.success('Farm Admin deleted.');
    } catch (err: any) {
      message.error(err?.data?.message || 'Failed to delete Farm Admin.');
      throw err;
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="cfg-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
        <span style={{ color: 'var(--text-muted)', fontSize: '16px' }}>Loading Farm Admins directory...</span>
      </div>
    );
  }
  ...
  return (
    <RolePanel
      ...
      records={draft.farmAdmins}
      newRow={draft.newFarmAdmin}
      onNewRowChange={(row) => updateDraft('newFarmAdmin', row)}
      onRecordsChange={(records) => updateDraft('farmAdmins', records)}
      onSave={() => {}}
      areas={draft.areas}
