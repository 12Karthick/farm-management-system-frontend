...
export default function FarmMastersTab() {
  ...
  // Map backend entities to compatible frontend RoleRecords
  const farmMastersList = useMemo(() => {
    if (!farmPersonsData) return [];
    return farmPersonsData
      .filter((fp) => fp.farmRole === 'Farm Master')
      .map((fp) => {
  ...
  // Sync loaded database farm masters with local draft state
  useEffect(() => {
    if (farmPersonsData) {
      if (JSON.stringify(draft.farmMasters) !== JSON.stringify(farmMastersList)) {
        updateDraft('farmMasters', farmMastersList);
      }
    }
  }, [farmPersonsData, farmMastersList, draft.farmMasters, updateDraft]);
  ...
  async function handleLiveCreate(rec: RoleRecord) {
    try {
      ...
      message.success(`Farm Master "${rec.name}" successfully created.`);
    } catch (err: any) {
      message.error(err?.data?.message || 'Failed to create Farm Master.');
      throw err;
    }
  }
  ...
  async function handleLiveEdit(rec: RoleRecord) {
    try {
      ...
      message.success(`Farm Master "${rec.name}" successfully updated.`);
    } catch (err: any) {
      message.error(err?.data?.message || 'Failed to update Farm Master.');
      throw err;
    }
  }
  ...
  async function handleLiveDelete(id: string) {
    try {
      ...
      message.success('Farm Master deleted.');
    } catch (err: any) {
      message.error(err?.data?.message || 'Failed to delete Farm Master.');
      throw err;
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="cfg-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
        <span style={{ color: 'var(--text-muted)', fontSize: '16px' }}>Loading Farm Masters directory...</span>
      </div>
    );
  }
  ...
  return (
    <RolePanel
      ...
      records={draft.farmMasters}
      newRow={draft.newFarmMaster}
      onNewRowChange={(row) => updateDraft('newFarmMaster', row)}
      onRecordsChange={(records) => updateDraft('farmMasters', records)}
      onSave={() => {}}
      areas={draft.areas}
