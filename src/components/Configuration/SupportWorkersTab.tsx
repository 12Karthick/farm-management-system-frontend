...
export default function SupportWorkersTab() {
  ...
  // Map backend entities to compatible frontend RoleRecords for Support Workers
  const supportWorkersList = useMemo(() => {
    if (!farmPersonsData) return [];
    return farmPersonsData
      .filter((fp) => fp.farmRole === 'Farm Support Worker')
      .map((fp) => {
  ...
  // Sync loaded database support workers with local draft state
  useEffect(() => {
    if (farmPersonsData) {
      if (JSON.stringify(draft.supportWorkers) !== JSON.stringify(supportWorkersList)) {
        updateDraft('supportWorkers', supportWorkersList);
      }
    }
  }, [farmPersonsData, supportWorkersList, draft.supportWorkers, updateDraft]);
  ...
  async function handleLiveCreate(rec: RoleRecord) {
    try {
      ...
      message.success(`Farm Support Worker "${rec.name}" successfully created.`);
    } catch (err: any) {
      message.error(err?.data?.message || 'Failed to create Farm Support Worker.');
      throw err;
    }
  }
  ...
  async function handleLiveEdit(rec: RoleRecord) {
    try {
      ...
      message.success(`Farm Support Worker "${rec.name}" successfully updated.`);
    } catch (err: any) {
      message.error(err?.data?.message || 'Failed to update Farm Support Worker.');
      throw err;
    }
  }
  ...
  async function handleLiveDelete(id: string) {
    try {
      ...
      message.success('Farm Support Worker deleted.');
    } catch (err: any) {
      message.error(err?.data?.message || 'Failed to delete Farm Support Worker.');
      throw err;
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="cfg-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '300px' }}>
        <span style={{ color: 'var(--text-muted)', fontSize: '16px' }}>Loading Farm Support Workers directory...</span>
      </div>
    );
  }
  ...
  return (
    <RolePanel
      ...
      addLabel="Add Support Worker"
      idLabel="Employee Code"
      records={draft.supportWorkers}
      newRow={draft.newSupportWorker}
      onNewRowChange={(row) => updateDraft('newSupportWorker', row)}
      onRecordsChange={(records) => updateDraft('supportWorkers', records)}
      onSave={() => {}}
      areas={draft.areas}
